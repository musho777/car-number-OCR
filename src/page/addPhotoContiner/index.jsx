...
export const AddPhotContiner = () => {
  ...
  const saveData = (name, text, text1, imageUri, upload) => {
    ...
    db.transaction(tx => {
      tx.executeSql(
        'INSERT INTO my_table (name, text, text1, imageUri, createat, upload) VALUES (?, ?, ?, ?, ?, ?)',
        [name, text, text1, imageUri, date, upload],
        () => {
          navigation.goBack();
        },
        (error) => {
  ...
  const handleSaveData = async () => {
    ...
    fetch("https://xn----nbck7b7ald8atlv.xn--y9a3aq/iosapp.loc/public/api/uploadImageCantainer", requestOptions)
      .then((response) => {
        response.json()
      })
      .then((result) => {
        saveData(name, value, value1, JSON.stringify(url), 1)
      })
      .catch((error) => saveData(name, value, value1, JSON.stringify(url), 0))
